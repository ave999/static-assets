...
.DESCRIPTION
    Launches an HTTP server on port 80 providing a browser-based form for deploying
    SCCM applications.

    Architecture:
      - HTTP server runs in a background runspace (no WMI access needed)
      - SCCM deployment runs on the MAIN THREAD — the interactive PowerShell session
        where the ConfigurationManager module connects correctly via WMI/DCOM

    Communication via a thread-safe synchronized hashtable ($SharedState).
